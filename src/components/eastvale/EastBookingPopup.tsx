...
const trackPopup = (action: 'open' | 'close' | 'cta_phone' | 'cta_whatsapp' | 'cta_form', trigger?: string) => {
  if (typeof window.gtag === 'function') {
    window.gtag('event', 'eastvale_popup', { action, trigger });
  }
};
...
const EastBookingPopup = () => {
  ...
  const handleForm = () => {
    trackPopup('cta_form');
    setOpen(false);
    scrollToLeadFormAfterModal('eastvale-lead-form');
  };
      ...
      {open ? (
        ...
        >
          ...
          <motion.div
            role="dialog"
            aria-modal="true"
            aria-labelledby="eastvale-popup-title"
          ...
          >
            <div className="relative bg-black px-6 py-5 text-white md:px-8 md:py-6">
              ...
              <h3 id="eastvale-popup-title" className="mt-3 text-2xl font-bold leading-tight md:text-3xl">
                {b.title}
              </h3>
